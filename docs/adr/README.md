...
| [095](095-a-simulation-moves-the-world-not-the-clock.md) | A simulation moves the world, not the clock — `cmd/sim` slides every stored instant into the past by shape and runs the server's own sweeps; the product carries no simulated time, no endpoint, no build tag; sign-in tables are named and kept still |
| [096](096-a-window-that-closes-settles-something.md) | A window that closes settles something — ordinary proposals get the hourly sweep elections always had; a vote that misses quorum at close lapses (`state = 'lapsed'`, worded "not decided", never "rejected"); a patch born under an elected template adopts it at birth and opens its first election *(applies 051, extends 092's read-time landing to the clock)* |
