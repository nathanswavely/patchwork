...
| [111](111-the-reader-says-how-wide-the-list-is.md) | The reader says how wide the list is — the cards pane gets three stops named in cards (two columns, one column, hidden), persisted like the collapsed rail rather than session-ephemeral like a lens; the control is the shell’s and migrates to canvas chrome at the hidden stop, because a control that can delete its own container was never the container’s; hiding suspends the in-view lens and a docked profile reopens the pane without overwriting the stop; 45% stops being a literal in three files *(amends 074, 094)* |
