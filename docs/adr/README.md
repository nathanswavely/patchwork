...
| [2026-09-20](2026-09-20-an-instance-vouches-for-an-app.md) | An instance vouches for an app: every magic-link email also carries a six-digit sign-in code that `POST /api/v1/auth/magic-link/verify` redeems exactly as the link does, for any client that cannot receive the link in its own session; a `native_apps` table, empty by default and step-up gated to add, is the only source of the Apple and Android association files at `/.well-known/`, so no instance vouches for an app its admins did not list; Android packages' `apk-key-hash` origins join the relying party; the table stays behind in a seamrip *(extends 017, 013, 025; applies 087's shape; the `applinks` block is deferred)* |
