...
- **Hetzner's port-25 block is a form, not a wall.** Outgoing 25/465 is
  ...
  request, decided case-by-case, usually within a day. quilt-1 qualifies.
...
lancasterpatchwork.org's mail or DNS.

1. **File the Hetzner limit request** for outgoing 25 on quilt-1 (account
...
2. **Buy a throwaway domain** (~$3–10). Not a subdomain of
   lancasterpatchwork.org: Gmail's reputation model weighs the organizational
   domain, so a test that goes badly must not splash the real one.
3. **Stand up Stalwart** as a compose sidecar on quilt-1 (or a scratch
...
- **The spike measures one IP.** quilt-1's result doesn't transfer to the
