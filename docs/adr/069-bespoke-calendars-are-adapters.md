...
- **The parsers stay format-generic.** ICS, schema.org JSON-LD, the one
  fixed-schema vendor shape, and an atproto collection are things a
  thousand sites emit identically. Nothing that requires knowing which
  site produced it belongs in the binary that every instance runs.
  (atproto arrived four days after this was drafted, in docs/adr/064, and
  is the rule's first live test rather than an exception to it: a lexicon
  fixes the schema, so one parser reads every repository publishing that
  collection — the Squarespace argument exactly.)
