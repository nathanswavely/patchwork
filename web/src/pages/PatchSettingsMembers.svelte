    ...
    {#if electedCouncil}
      <p class="council-note muted">
        ...
        {:else if nextContestOpens}
          This patch elects its council, so admins are not made here. All {councilSeats.length} seat{councilSeats.length === 1 ? '' : 's'} {councilSeats.length === 1 ? 'is' : 'are'} held; the next election opens {formatDay(nextContestOpens)}. To make room sooner, add a seat on the Governance page, or bring a seat's term end forward there.
        {:else}
          This patch elects its council, so admins are not made here. Every seat is held and no election is scheduled. To make room, add a seat on the Governance page.
        {/if}
      </p>
    {/if}
