<script>
  ...
  import { getQuery, navigate } from '../stores/router.svelte.js';
  import {
  ...
  let token = $derived(query.get('token') || '');
  ...
  $effect(() => {
    if (token) {
      ...
      validating = false;
      validationError = 'Missing signup token';
    }
  });
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      submitError = e.message || 'Signup failed';
    } finally {
      submitting = false;
    }
  }
    ...
    {:else if !valid}
      <div class="card">
        ...
        <p class="muted" style="margin-top: 1rem;">
          No account was created yet. Request a new sign-in link and
      ...
      <div class="card">
        ...
        <form onsubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
          ...
          <input
            ...
            type="text"
            bind:value={username}
            placeholder="e.g. gallery-row-gal"
          ...
          {:else}
            <p class="hint muted">3-30 characters: lowercase letters, numbers, hyphens.</p>
          {/if}
