...
- **Address suggestions:** an optional local **gazetteer** — an OpenStreetMap extract cropped to the instance radius, built offline by `cmd/gazetteer` and copied in as a SQLite file (docs/adr/082). No geocoding service, no key, nothing fetched at runtime. It only ever *proposes* a marker; a coordinate is saved when a person confirms it, so an instance without the file behaves exactly as before
...
- `POST /api/v1/events` — members/admins post directly; anyone else submits for review (`status: pending_review`) per docs/adr/026; trusted contributors (users flag) post directly to unclaimed patches. An event names **who it is for** — `visibility` is `public`, `followers` or `members`, and the patch's `follower_permissions.events` is a ceiling read down over it in Go on every read path, never written to the row (docs/adr/2026-09-19-an-event-says-who-it-is-for-within-what-the-patch-allows.md); a non-member's submission is always `public`, because a stranger does not place an event inside a patch's private calendar. `event_url` is the event's **own page out on the web** (docs/adr/079) — tickets, the venue's listing — distinct from the Patchwork permalink and from an `event_link`. http(s) only, checked at every write path *and* at every parser, because it is rendered as an href and a feed is somebody else's input
...
- `GET|POST /api/v1/nodes/{slug}/event-sources`, `DELETE|POST .../{id}[/sync]`, `POST /api/v1/events/{id}/detach` — event sources (docs/adr/031): owner-attached calendar feeds (ICS; Squarespace and schema.org-marked events pages auto-detected), synced hourly; imported events publish directly, are read-only until detached. A source carries a **default tier** applied at INSERT only — the one field the sync never refills, because who an event is for is local policy rather than something the feed owns, so a per-event change sticks across syncs and a non-public default stops those events federating. Every parser fills `Item.URL` and the reconciler diffs it, so **adding a field the feed is authoritative about needs no backfill migration** — the next sync sees every row as changed and fills it (docs/adr/079). Detach does not clear it: detach severs provenance, and the link is content
...
- `POST /api/v1/auth/magic-link/verify` — finishing a magic-link sign-in by **code**: the same email carries six digits beside the link, for a client that cannot receive that link in its own session (a native app, a CLI, a second device), so the proof is typed rather than clicked. Same row, same 15 minutes, and same answers as the link's JSON branch — the session cookie, or `username_required` plus a signup token for an address with no account (docs/adr/013). Five wrong codes mark the row used, which burns the emailed link too: both prove the one thing, and a six-digit secret has to stop answering long before a guesser gets anywhere. Every failure is one indistinguishable `invalid or expired code`, so the endpoint cannot be asked whether an address was ever asked for here
...
- `GET|POST /api/v1/admin/native-apps`, `DELETE /api/v1/admin/native-apps/{id}`, plus the two public files `GET /.well-known/apple-app-site-association` and `GET /.well-known/assetlinks.json` — the **native apps this quilt vouches for** (docs/adr/2026-09-20-an-instance-vouches-for-an-app.md). A phone hands a passkey to an app only if the domain publishes the app's identifier, so the identifiers are a table an instance admin fills in and the two files are rendered from it. **Empty by default and empty is a 404**: a quilt that has listed nothing publishes neither file, and the project hardcodes no app identifier anywhere. Adding is step-up gated (docs/adr/017), because publishing an identifier lets that app invoke the domain's passkeys — the same class of act as setting a user's email (docs/adr/072); removing is admin-only, since taking trust back is the safe direction. Audited `admin.native_app_add` / `admin.native_app_remove`. Both files are mounted **outside the federation gate**, like `/api/v1/instance/attestation-key`: an app store's fetcher has no account and does not care whether the instance federates. The Apple file carries `webcredentials` and deliberately no `applinks`. An Android assertion arrives from `android:apk-key-hash:…` rather than the site's origin, so `auth.NativeAppOrigins` derives those origins from the listed fingerprints and `WebAuthnService.Reconfigure` reloads them at startup and after every add and remove — a listed app that signs in against a stale origin list fails at the last step of a ceremony that otherwise worked. The table stays behind in a seamrip (a fork answers at another domain and lists its own apps)
