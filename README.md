...
- **Governance, when you want it.** Proposals (*baste requests*) run on a
  real state machine (discussion → voting → in effect), with threaded
  discussion, revisions, and vote tallies. Governance documents (*linings*)
  live in per-patch **git repositories**, full version history and diffs
  included. A band never has to touch any of it. A coalition can run itself
  entirely on it. Every new patch ships with an anti-discrimination baseline
  lining.
...
textile (patches, baste requests, linings). The mapping lives in
